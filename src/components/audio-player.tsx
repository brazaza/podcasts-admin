...
import { Play, Pause, SkipBack, SkipForward, Shuffle, Repeat, Volume2, X, ChevronDown, ChevronUp } from "lucide-react";
...
export function GlobalPlayer() {
  ...
  const artistNames = currentPodcast.artists?.map((a: { name: string }) => a.name).join(" b2b ") ?? "";
                    ...
                    <div className="flex items-center justify-between gap-3 md:gap-8 h-full">
                      {/* Left: Podcast Info */}
                        <div className="flex items-center gap-2 md:gap-4 md:w-[200px] lg:w-[350px] shrink-0 min-w-0 flex-1 md:flex-none">
                          <Link href={`/podcast/${currentPodcast.number}`} className="shrink-0">
                            <img src={currentPodcast.coverImage ?? ""} alt={currentPodcast.title} className="w-9 h-9 md:w-12 md:h-12 object-cover border border-white/10 hover:opacity-80 transition-opacity" />
                          </Link>
                                <div className="overflow-hidden min-w-0">
                                  <Link href={`/podcast/${currentPodcast.number}`} className="block truncate group/title w-fit relative text-yellow-400 transition-colors">
                            ...
                            {(currentPodcast.artists ?? []).map((a: { name: string; slug: string }, i: number, arr: { name: string; slug: string }[]) => (
                              <React.Fragment key={a.slug}>
                                ...
                                >
                                  ...
                                  {i < arr.length - 1 && (
                                    <span className="text-[7px] md:text-[9px] text-white font-bold uppercase whitespace-nowrap">b2b</span>
                                  )}
                              </React.Fragment>
                            ))}
                          </div>
                      ...
                      <div className="flex items-center justify-center gap-2 md:gap-6 shrink-0">
                        <Button 
                          ...
                          className={cn("text-zinc-500 hover:text-white transition-colors hidden lg:flex", isShuffle && "text-white")}
                        ...
                        <Button 
                          variant="ghost" size="icon" 
                          className={cn("text-zinc-500 hover:text-white transition-colors hidden lg:flex", isRepeat && "text-white")}
                      ...
                      <div className="flex items-center justify-end gap-2 md:gap-4 md:w-[200px] lg:w-[350px] shrink-0">
                        ...
                        <div className="flex items-center gap-1 border-l border-white/10 pl-2 md:pl-4">
                          <Button variant="ghost" size="icon" className="text-zinc-400 hover:text-white h-8 w-8 transition-all active:scale-90" onClick={() => setIsMinimized(true)} title="Minimize">
                            <ChevronDown className="w-5 h-5" />
                          </Button>
                          <Button variant="ghost" size="icon" className="text-zinc-400 hover:text-red-500 h-8 w-8 transition-transform active:scale-90" onClick={stop} title="Close">
                            <X className="w-4 h-4" />
                          </Button>
                        </div>
                      </div>
                  ...
                  <div className="max-w-[1400px] mx-auto px-4 md:px-6 h-full">
                    <div className="flex items-center justify-between gap-4 w-full h-full">
                              ...
                              <div className="flex-1 overflow-hidden relative h-full flex items-center [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
                                  <div className="flex whitespace-nowrap overflow-hidden w-full h-full items-center">
                                    <div className="animate-marquee flex shrink-0 items-center">
                                      {[...Array(6)].map((_, i) => (
                                        <span key={i} className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em] text-yellow-400 pr-8 transition-colors">
                                          {podcastLabel}
                                        </span>
                                      ))}
                                    </div>
                      ...
                      <div className="flex items-center justify-end md:w-[200px] lg:w-[350px] shrink-0">
                        <div className="flex items-center gap-1 border-l border-white/10 pl-2 md:pl-4">
                           <Button variant="ghost" size="icon" className="text-zinc-400 hover:text-white h-8 w-8 transition-all active:scale-90" onClick={(e) => {
                             e.stopPropagation();
                             setIsMinimized(false);
                           }}>
                            <ChevronUp className="w-5 h-5" />
                          </Button>
                          <Button variant="ghost" size="icon" className="text-zinc-400 hover:text-red-500 h-8 w-8 transition-transform active:scale-90" onClick={(e) => {
                            e.stopPropagation();
                            stop();
                          }}>
                            <X className="w-4 h-4" />
