...
function getAudioUrl(podcast: AudioPodcast): string {
  if (podcast.audioFile) return podcast.audioFile;
  if (podcast.audio_url) return podcast.audio_url;
  return '';
}

export function AudioProvider({ children, podcasts }: { children: React.ReactNode, podcasts: AudioPodcast[] }) {
  ...
  const play = useCallback(async (podcast: AudioPodcast) => {
