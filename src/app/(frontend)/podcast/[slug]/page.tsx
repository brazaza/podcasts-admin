...
export default async function PodcastPage({ params }: PageProps) {
  ...
  const getTitleSize = () => {
    ...
    if (longestWord >= 12 || titleLength > 25)
      return 'text-xl sm:text-2xl md:text-3xl lg:text-3xl xl:text-4xl'
    if (longestWord >= 10 || titleLength > 20)
      return 'text-2xl sm:text-3xl md:text-4xl lg:text-4xl xl:text-5xl'
    if (longestWord >= 8 || titleLength > 15)
      return 'text-3xl sm:text-4xl md:text-5xl lg:text-5xl xl:text-6xl'
    if (longestWord >= 7 || titleLength > 12)
      return 'text-4xl sm:text-5xl md:text-6xl lg:text-6xl xl:text-6xl'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start mb-24 relative">
        ...
        <div className="space-y-8 min-w-0 overflow-hidden">
          <div className="space-y-4">
            <h1
              className={`${getTitleSize()} font-black tracking-tighter uppercase leading-[0.9] font-display group/title w-fit text-yellow-400 transition-colors`}
            >
      ...
      {artists.length > 0 && (
        <section>
          ...
          >
            {artists.map((artist) => {
              const bannerUrl =
                typeof artist.banner_image === 'object' && artist.banner_image?.url
